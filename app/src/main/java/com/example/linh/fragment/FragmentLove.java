...
import com.example.linh.adapter.LoveAdapter;
...
public class FragmentLove extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new LoveAdapter(new Interface() {
            @Override
    ...
    public void onResume() {
        super.onResume();
        List<Contact> mlist = db.getLove();
        adapter.setList(mlist);
    }

    private void askPermissionAndCall(Contact c) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            int result = ActivityCompat.checkSelfPermission(getContext(),
